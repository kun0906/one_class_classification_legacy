...
def initialize_c_as_mean(nnet, n_batches, eps=0.1):
    ...
    if isinstance(n_batches, str) and n_batches == "all":
...
def train_autoencoder(nnet):
    ...
    for epoch in range(nnet.ae_n_epochs):
        ...
        batches = 0
        train_err = 0
        train_scores = np.empty(int(nnet.data.n_train))
        ...
        for batch in nnet.data.get_epoch_train():
            ...
            train_scores[int(start_idx):int(stop_idx)] = b_scores.flatten()
            batches += 1
    ...
    if nnet.data._X_train.ndim == 4:
        ...
        n_img = 32
        random_idx = np.random.choice(int(nnet.data.n_train), n_img, replace=False)
